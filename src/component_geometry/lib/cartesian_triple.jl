...
show(io::IO, a::CartesianTriple{T,U}) where {T<:Number, U<:ComponentType} =
...
promote_rule(::Type{Point3{T}}, ::Type{Vector3{U}}) where {T<:Number, U<:Number} =
    Point3{promote_type(T,U)}

promote_rule(::Type{CartesianTriple{S, U}}, ::Type{CartesianTriple{T, U}}) where {S<:Number, T<:Number, U<:ComponentType} =
    CartesianTriple{promote_type(S, T), U}

promote_rule(::Type{CartesianTriple{S, U}}, ::Type{T}) where {S<:Number, T<:Number, U<:ComponentType} =
    CartesianTriple{promote_type(S, T), U}
